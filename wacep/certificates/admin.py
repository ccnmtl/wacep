...
def duct_tape_certificate_function(course, user_obj, function_name):
    def certificate_function(user):
        ...
        if course in [ cert.course for cert in user.certificates_earned.all()]:
            alt_text = cert.__unicode__()
            return '<img src="/media/admin/img/icon-yes.gif" alt="%s">' % alt_text
        else:
            alt_text = '%s did not take this class' % user.__unicode__()
            return '<img src="/media/admin/img/icon-no.gif" alt="%s">' % alt_text
